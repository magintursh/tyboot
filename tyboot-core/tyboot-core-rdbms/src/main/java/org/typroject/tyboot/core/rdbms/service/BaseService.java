...
public class BaseService<V extends BaseModel, P extends BaseEntity, M extends BaseMapper<P>> extends ServiceImpl<M, P> implements IService<P> {
    ...
    private boolean refreshCache = true;//每次从缓存中查询到数据，对缓存的时间进行重置
    private static String valueOfNullModel = "NULL";
    ...
    public void setCacheNullModel(boolean cacheNullModel) {
        this.cacheNullModel = cacheNullModel;
    }
    ...
    protected final V queryByModelWithCache(V model, String propertyValueAsCacheKey) {
        Object v = queryFromCache(genCacheKeyForModel(propertyValueAsCacheKey));
        if (ValidationUtil.isEmpty(v)) {
    ...
    protected final V queryModelByParamsWithCache(Object... params) {
        ...
        if (ValidationUtil.isEmpty(v)) {
            ...
            if (ValidationUtil.isEmpty(v) && cacheNullModel)
                saveCache(genCacheKeyForModel(Redis.genKey(asKey)), valueOfNullModel);
        }
    ...
    protected final List<V> queryForListWithCache(String orderBy, boolean isAsc, Object... params) {
        ...
        Object list = queryFromCache(cacheKey);
        if (ValidationUtil.isEmpty(list)) {
            ...
            if (ValidationUtil.isEmpty(list) && cacheNullModel)
                saveCache(cacheKey, valueOfNullModel);
        }

        if (valueOfNullModel.equals(list))
