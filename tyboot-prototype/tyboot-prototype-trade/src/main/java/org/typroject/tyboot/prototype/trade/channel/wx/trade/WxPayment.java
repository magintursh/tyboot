...
import org.typroject.tyboot.prototype.trade.channel.DefaultChannelType;
...
public class WxPayment implements Trade {
	...
	private String configur(TransactionsSerialModel serialModel, Map<String, Object> extra) throws Exception
	{
		...
		ChannelType channelType = (ChannelType)extra.get("channelType");
        ...
        params.put("body", (String) extra.get(PropertyConstants.PAYMENT_SUBJECT));//商品描述
        ...
        //params.put("total_fee",serialModel.getTradeAmount().multiply(new BigDecimal(100)).intValue());//总金额
        params.put("total_fee", new BigDecimal(1).intValue());//总金额
        ...
        params.put("trade_type", channelType.getChannel());//交易类型	APP
        params.put("openid", extra.get("openId"));
    ...
    public TradeResultModel process(TransactionsSerialModel serialModel, Map<String, Object> extra) throws Exception {
		...
		if(!ValidationUtil.isEmpty(resultXml))
		{
			...
			if("SUCCESS".equals(map.get("return_code")) && "SUCCESS".equals(map.get("result_code")))
			{
				...
				if(DefaultChannelType.WX_JSAPI.getChannel().equals(channelType.getChannel()))
				{
					...
					params2Sign.put("paySign",sign(params2Sign));
					resultModel.setResult(params2Sign);
				}


				if(DefaultChannelType.WX_NATIVE.getChannel().equals(channelType.getChannel()))
				{

					params2Sign.put("code_url",map.get("code_url"));
					resultModel.setResult(params2Sign);
				}
