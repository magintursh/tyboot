...
public class AccountSerialService extends BaseService<AccountSerialModel, AccountSerial, AccountSerialMapper>
{
    ...
    public AccountSerialModel queryLastAccountSerial(String accountNo) throws Exception
    {
        ...
        List<AccountSerial> entitis = this.list(wrapper);

        //@TODO 限制返回数据量
        if(!ValidationUtil.isEmpty(entitis))
        {
            AccountSerial first = entitis.get(0);
            accountSerialModel = Bean.toModel(first,new AccountSerialModel());
        }
    ...
    public Page querySerialPage(Page page, String accountNo, String accountType, String userId, String bookkeeping)throws Exception
    {
       return this.queryForPage(page,"OPERATE_TIME",false,accountNo, accountType, userId, bookkeeping);
    }
