...
public class EmqKeeper {
    ...
    public EmqKeeper(EmqProperties emqProperties) throws Exception {
        ...
        options.setKeepAliveInterval(emqProperties.getKeepAliveInterval());// 设置会话心跳时间 单位为秒 服务器会每隔1.5*20秒的时间向客户端发送个消息判断客户端是否在线，但这个方法并没有重连的机制
        options.setMaxInflight(emqProperties.getMaxInflight());
